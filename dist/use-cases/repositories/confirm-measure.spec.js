...
describe('Confirm Measure Use Case', () => {
    ...
    it('shoud not be able to confirm measure not created', async () => {
        ...
        expect(async () => await sut.execute({ id, confirmed_value: 12345 })).rejects.toBeInstanceOf(MeasureNotFoundError);
    });
    it('shoud not be able to confirm measure has confirmed', async () => {
        ...
        await sut.execute({ id, confirmed_value: 12345 });
        expect(async () => await sut.execute({ id, confirmed_value: 12345 })).rejects.toBeInstanceOf(ConfirmationDuplicateError);
    });
});
