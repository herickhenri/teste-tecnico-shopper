...
describe('Get Measure Use Case', () => {
    ...
    it('shoud not be able to get measurements  if the customer does not exist', async () => {
        const customer_code = 'example-customer-code';
        expect(async () => await sut.execute({ customer_code })).rejects.toBeInstanceOf(MeasuresNotFoundError);
    });
    it('shoud not be able to get measurements if can`t find any measure', async () => {
        ...
        expect(async () => await sut.execute({ customer_code, type: 'GAS' })).rejects.toBeInstanceOf(MeasuresNotFoundError);
    });
